...
class SchedulerEntrypoint(consumers.MessageHandler):
    ...
    def process(self, request):
        ...
        LOG.info('Received a new LiveAction request from RabbitMQ: {}'.format(str(request.id)))
        try:
        ...
        LOG.info('Checking the ActionExecutionSchedulingQueue to see if the action exists: {}'.format(str(liveaction_db.id)))
        queued_requests = ActionExecutionSchedulingQueue.query(**query)
        LOG.info('Retrieved {} actions from the ActionExecutionSchedulingQueue for id: {}'.format(len(queued_requests), str(liveaction_db.id)))
        ...
        LOG.info('Creating execution queue item for liveaction with id: {}'.format(str(liveaction_db.id)))
        execution_queue_item_db = self._create_execution_queue_item_db_from_liveaction(
            liveaction_db,
            delay=liveaction_db.delay
        )
        LOG.info('Finished creating execution queue item for liveaction with id: {}'.format(str(liveaction_db.id)))

        LOG.info('Adding execution queue item to queue with  id: {}'.format(str(liveaction_db.id)))
        ActionExecutionSchedulingQueue.add_or_update(execution_queue_item_db, publish=False)
        LOG.info('Finished adding execution queue item to queue with  id: {}'.format(str(liveaction_db.id)))
