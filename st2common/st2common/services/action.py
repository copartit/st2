...
def publish_request(liveaction, execution):
    ...
    LOG.info('LiveAction.publish_create took {} seconds'.format((after - before).total_seconds()))
    before = datetime.datetime.now()
    LiveAction.publish_status(liveaction)
    after = datetime.datetime.now()
    LOG.info('LiveAction.publish_status took {} seconds'.format((after - before).total_seconds()))
    before = datetime.datetime.now()
    ActionExecution.publish_create(execution)
    after = datetime.datetime.now()
    LOG.info('ActionExecution.publish_create took {} seconds'.format((after - before).total_seconds()))
